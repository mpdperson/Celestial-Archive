...
<script>
	...
	export default defineComponent({
		...
		setup (props) {
			...
			const bookVis = ref(!true)
			const rejectVis = ref(true)
			const acceptVis = ref(true)
			...
			const getDisplayList = async () => {
				displayList.value = await Store.fetchTempPerks();
			...
			return {
				...
				roll() {
					...
					if(randomPerk.Add) {
						...
						if(
							isNull(actPerk.Prereq_Title) 
							&& isNull(actPerk.Restrict_Title) 
							&& isNull(actPerk.Exclude_Title)
						) {
